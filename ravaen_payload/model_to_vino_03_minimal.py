...
"""
try:
    predict_function = load_model("encoder_model.onnx", device='MYRIAD')

    example_input = np.random.rand(1, 4, 32, 32)
    print("example_input", example_input.shape)

    example_output = predict_function(example_input)
    print("example_output", example_output.shape)
except Exception as e:
    print("1 failed MYRIAD,", e)

try:
    predict_function = load_model("encoder_model.onnx", device='CPU')
